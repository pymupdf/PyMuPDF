...
int _as_dict(PyObject *block_list, fz_stext_block *block, fz_buffer *text_buffer, int raw, fz_rect tp_rect, int block_n, int dev_flags)
{
    /*
    Recursive function for output by blocks as identified by the
    MuPDF SEGMENT logic.
        ...
        switch (block->type)
        {
            case FZ_STEXT_BLOCK_STRUCT:
                if (block->u.s.down && block->u.s.down->first_block)
                {
                    ...
                    block_n = _as_dict(subblocks, block->u.s.down->first_block, text_buffer, raw, tp_rect, block_n, dev_flags);
...
void _getTableBlocks(fz_stext_page *tp, PyObject *table_dict, PyObject *bbox, int dev_flags)
{
    fz_context* ctx = mupdf::internal_context_get();
    fz_rect bounds = JM_rect_from_py(bbox);
    fz_stext_block *block;
    ...
    {
        int block_n = -1;
        block_n = _as_dict(block_list, block, text_buffer, 0, bounds, block_n, dev_flags);
        DICT_SETITEM_DROP(table_dict, dictkey_blocks, block_list);
    }
    else
    {
        DICT_SETITEM_DROP(table_dict, dictkey_blocks, block_list);
...
void JM_make_textpage_dict(fz_stext_page *tp, PyObject *page_dict, int raw, int dev_flags)
{
    ...
    int block_n = -1;
    block_n = _as_dict(block_list, block, text_buffer, raw, tp_rect, block_n, dev_flags);
