...
def insert_htmlbox(
    ...
    *,
    ...
    def story_maker(clip, text, scale=True, rotate=0, css=css, archive=archive):
        ...
        # ---------------------------------------------------------------------
        filled, factor = story.scale_to_fit(width=rect.width, height=rect.height, expand=scale, shrink=False)
        ...
        doc = fitz.Document("pdf", fp)  # reopen memory PDF
        return (
            doc,
            filled,
            factor,
            (rect * fitz.Matrix(-rotate)).torect(clip),
            links,
        )
    ...
    for uri, rect in links:
        rect = fitz.Rect(rect) * fitz.Matrix(-rotate) * matrix
        if uri.startswith("name:"):
            link = {"kind": fitz.LINK_NAMED, "name": uri[5:], "from": rect}
        else:
            link = {"kind": fitz.LINK_URI, "uri": uri, "from": rect}
